...
class InventoryListCreateView(APIView):
    ...
    def get(self, request: Request, *args, **kwargs) -> Response:
        serializer = self.serializer_class(self.get_queryset(request.query_params), many=True)

        return Response(serializer.data, status=200)

    def get_queryset(self, query_params):
        # get the filter date (in ISO8601 format)
        date_filter = query_params.get('date')
        if date_filter:
            date = parse_datetime(date_filter)
            if date is not None:
                return self.queryset.filter(metadata__year__gt=date.year)
            else:
                raise ValidationError(
                    {"error": "Invalid date format, please use ISO 8601 format, e.g., 2002-11-04T00:00:00Z"},
                )
        else:
